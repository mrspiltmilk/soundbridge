...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-slate-50 dark:bg-[#050505] transition-colors duration-300">
          ...
          {state === AppState.ERROR && (
            <div className="text-center space-y-6 animate-in zoom-in-95 duration-300">
              <div className="p-4 bg-red-500/5 dark:bg-red-500/10 border border-red-500/20 rounded-2xl">
                <p className="text-red-600 dark:text-red-400 text-xs font-semibold tracking-wide">{error}</p>
              </div>
              <button
                onClick={() => setState(AppState.IDLE)}
                className="text-[10px] font-bold uppercase tracking-[0.3em] text-slate-600 dark:text-neutral-400 hover:text-fuchsia-500 transition-colors"
              >
                Try again
              </button>
            </div>
      ...
      <footer className="fixed bottom-8 text-[9px] uppercase tracking-[0.4em] text-slate-400 dark:text-neutral-600 pointer-events-none">
        Gemini 3 Pro • Minimal Interface
      </footer>
    </div>
  );
};
